...
function createBackground(src, index) { // Create the background
    ...
    // The image
    let img = new Image();
    img.src = `/images/backgrounds/${src}`;
...
function loadFrame(evt, isImport) {
    ...
    // Set the background
    main.style.background = frames[currentFrame].background ? `url("/images/backgrounds/${frames[currentFrame].background}")` : 'white';
